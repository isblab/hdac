...
root_hier, dof = bs.execute_macro(max_rb_trans = 0.75,
                                  max_rb_rot = 0.1,
                                  max_bead_trans = 4.5,
                                  max_srb_trans = 0.5,
                                  max_srb_rot = 0.2)
...
set1 = []
for prot in ["KDM1A"]:
    set1 += IMP.atom.Selection(root_hier, molecule=prot, copy_index=0, residue_indexes=range(171,836)).get_selected_particles()
for prot in ["RCOR1"]:
    set1 += IMP.atom.Selection(root_hier, molecule=prot, copy_index=0, residue_indexes=range(311,443)).get_selected_particles()

fixed_set1_beads,fixed_set1_rbs = dof.disable_movers(set1, [IMP.core.RigidBodyMover, IMP.pmi.TransformMover])
...
xls = IMP.pmi.restraints.crosslinking.CrossLinkingMassSpectrometryRestraint(
                ...
                label = "xl_coREST_hdac1",                        #   to bias crosslinks towards each other
...
IMP.pmi.tools.shuffle_configuration(root_hier,
                                    ...
                                    excluded_rigid_bodies=fixed_set1_rbs,
                                    hierarchies_included_in_collision=set1)
