...
def get_intersection( x, y ):
    ...
    for x_ in x:
        ...
        if x != [] and len( x_ ) > len( y_ ):
            ...
            continue
    # False if there is no intersection else True.
...
if __name__ == '__main__':
    ...
    sizes_dict = {"HDAC1":483, "MBD3": 292, "GATAD2B":282, "RBBP4":426, "MTA1":716} 
    proteins = {
    "HDAC1-MTA1": {
    "HDAC1": [(8,376)],
    "MTA1": [(165,333)]
    ...
    # w = open(f"{p1name}.{copy1}-{p2name}.{copy2}_Contacts.csv", "w")

    # for k in distances.keys():
    #     if distances[k] <= 10:
    #         w.writelines(str(k.split("--")[0]) + "\t" + str(k.split("--")[1]) + "\t" + str(distances[k]) + "\n")
