...
class Align_Paralogs():
	def __init__(self):
		self.hdac = "HDAC1"
		self.exclude = True    # If True, excludes the other paralog.
		self.remove = "HDAC1" if self.hdac == "HDAC2" else "HDAC2"
		self.data = pd.read_csv("NuRD Complex Interactions.xlsx - NuRD Member XL Sites.csv")
		self.fasta_path = "./Uniprot/"
		self.output_path = "./EMBOSS/Needle/"
		self.outputfile_name = "Mapped_XLs_HDAC1"
		self.included_proteins = ["CHD", "GATA", "HDAC", "MBD", "MTA", "RBBP"]
		self.mapping = {i:[] for i in ["Protein 1", "Residue 1", "Protein 2", "Residue 2"]}
		...
		for i in range( len( self.data["Protein 1"] ) ):
			if self.exclude and ( self.remove in self.data["Protein 1"][i] or self.remove in self.data["Protein 2"][i] ):
				...
				if prot1 in dom_prot1:
					self.mapping["Protein 1"].append( prot1 )
					self.mapping["Residue 1"].append( self.data["Residue 1"][i] )
				# Consider either of HDAC1/HDAC2 XL and exclude the other.
				elif self.hdac in prot1 and self.exclude:
					self.mapping["Protein 1"].append( prot1 )
					self.mapping["Residue 1"].append( self.data["Residue 1"][i] )			
				else:
					self.mapping["Protein 1"].append( dom_prot1 )
					self.mapping["Residue 1"].append( [pos[1] for pos in alignments_dict[dom_prot1][prot1] if pos[0] == self.data["Residue 1"][i]][0] )

				prot2 = self.data["Protein 2"][i].replace(" ", "")
				dom_prot2 = self.return_dominant_paralog( prot2 )

				if prot2 in dom_prot2:
					self.mapping["Protein 2"].append( prot2 )
					self.mapping["Residue 2"].append( self.data["Residue 2"][i] )
				# Consider either of HDAC1/HDAC2 XL and exclude the other.
				elif self.hdac in prot2 and self.exclude:
					self.mapping["Protein 2"].append( prot2 )
					self.mapping["Residue 2"].append( self.data["Residue 2"][i] )
				else:
					self.mapping["Protein 2"].append( dom_prot2 )
					self.mapping["Residue 2"].append( [pos[1] for pos in alignments_dict[dom_prot2][prot2] if pos[0] == self.data["Residue 2"][i]][0] )

Align_Paralogs().forward()
